...
compliant device (e.g., a filesystem and \code{java.net}). Some
...
\code{iload} and \code{iadd} executes in two cycles, which means
that each of these two operations is executed in a single cycle. The
\code{iinc} bytecode is one of the few instructions that do not
manipulate the stack and can be measured alone. As \code{iinc} is
...
observation that the dynamic instruction count for \code{iinc} is
...
(\code{iadd} + \code{iload\_n}). We assume that we have to add some
