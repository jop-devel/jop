...
public class FileOutputStream {
	...
	public FileOutputStream(String file, boolean append) throws IOException {
		...
		try {
			entryBuffer = FatItS.fat_search_file(dirCluster, filename, entryBuffer);
		} catch (FileNotFoundException exc) {
			entryBuffer = FatItS.fat_add_file(dirCluster, filename, entryBuffer);
		}
		...
		if (append == false) {
			size = 0;
			entryBuffer = FatItS.fat_mod_file(dirCluster, filename, size, attribs, entryBuffer);
		} else {
			offset = size;
		}
	}
	...
	public void write(byte[] b, int off, int len) throws IOException {
		...
		if (((offset + (long) (off + len))) > (size + 1)) {
			...
			if (((offset + (long) (off + len)) / FatItS.BlockSize) < (size / FatItS.BlockSize)) {
				...
				for (int i = 0; i < diff; i++) {
					FatItS.fat_grow_file(startcluster);
				}
		...
		for (int i = 0; i < ((len / FatItS.BlockSize) + 1); i++) {
			...
			FatItS.fat_write_file(startcluster, buffer, block);
			...
			if (size <= offset) {
				...
				entryBuffer = FatItS.fat_mod_file(dirCluster, filename, size-1, attribs, entryBuffer);
			}
