...
public class CAMFactory extends IOFactory
{
	private CAM CAM;

	// Handles should be the first static fields!
	private static int CAM_PTR;
	private static int CAM_MTAB;

	CAMFactory() {
		CAM = (CAM) makeHWObject(new CAM(),
				Const.CAM_BASE, 0);
	...
	static CAMFactory single = new CAMFactory();
	
	public static CAMFactory getCAMFactory() {		
		return single;
	}

	public CAM getCAM() { return CAM; }
