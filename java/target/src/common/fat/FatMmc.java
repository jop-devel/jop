...
public class FatMmc implements FatLowLevel {
	...
	public void Flush() {
		// No buffered blocks for MMC, so do nothing
	}
	...
	public void ClearMedium() {
		int[] zeros = new int[512];
		
		// FIXME: Use correct number of sectors
		for (int sector = 0; sector < 150; sector++) {
			if (sector % 100 == 0)
				System.out.println("Clearing sector " + sector);
			WriteSector(sector, zeros);
		}

	}

	public int GetTotalBytes() {
		// TODO how to read sd card size?
		return 32*1024*1024; // assuming 32MB for testing
	}
}
