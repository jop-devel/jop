...
*/
	private static void wr(int data, int reg) {
		...
		JopSys.wr(reg | ISA_DIR, IO_ISA_CTRL);			// addr and drive data out
		JopSys.wr(reg | ISA_WR | ISA_DIR, IO_ISA_CTRL);	// niow low
		JopSys.wr(reg | ISA_DIR, IO_ISA_CTRL);			// niow high again
...
*/
	private static int rd(int reg) {
		...
		JopSys.wr(reg | ISA_RD, IO_ISA_CTRL);			// nior low
		int ret = JopSys.rd(IO_ISA_DATA);				// read data
		JopSys.wr(reg, IO_ISA_CTRL);					// nior high again
