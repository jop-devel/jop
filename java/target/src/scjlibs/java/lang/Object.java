...
*/
   ...
   */
	public String toString() {
		...
		/*
		 * This method creates a new StringBuilder to perform the concatenation.
		 * The returning String lives in the scope of the caller.
		 */
...
*/
  public final Class<?> getClass() {
	  ...
	  GC.log("getting class...");
	  
	  // Pointer to the handler of this object or array instance
	  int ptr = Native.toInt(this);
	  int type = Native.rdMem(ptr+GC.OFF_TYPE);

	  // pointer to the first element in case of an array
	  int arrayElement = Native.rdMem(ptr);
	  int arrayElementMethodAddress;
	  ...
	  if(type == 0){
		  methodsAddress = Native.rdMem(ptr+GC.OFF_MTAB_ALEN);
		  classRefAddress = methodsAddress - 6;
		  classObjRef = Native.rdMem(methodsAddress-1);
		  Class classObject = (Class) Native.toObject(classObjRef);
		  classObject.classRefAddress = classRefAddress;
		  return classObject;	  

	  }else{
		  arrayElementMethodAddress = Native.rdIntMem(Native.rdMem(arrayElement)+GC.OFF_MTAB_ALEN);
		  classRefAddress = arrayElementMethodAddress - 6;
