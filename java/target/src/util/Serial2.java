...
//
			i = wrptRx;
			j = rdptRx;
			while ((Native.rd(Native.IO_STATUS3) & Native.MSK_UA_RDRF)!=0 && ((i+1)&BUF_MSK)!=j) {
				rxBuf[i] = Native.rd(Native.IO_UART3);
				i = (i+1)&BUF_MSK;
			}
			wrptRx = i;
//
...
//
			i = rdptTx;
			j = wrptTx;
			while ((Native.rd(Native.IO_STATUS3) & Native.MSK_UA_TDRE)!=0 && i!=j) {
				Native.wr(txBuf[i], Native.IO_UART3);
				i = (i+1) & BUF_MSK;
			}
			rdptTx = i;
		}
